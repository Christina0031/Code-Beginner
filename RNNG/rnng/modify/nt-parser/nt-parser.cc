...
int main(int argc, char** argv) {
  ...
  if (conf.count("test_data")) {
    ...
    cerr << "Loading test set finished\n";
  }
  ...
  if (conf.count("train")) {
	...
	std::string command="python2 remove_dev_unk.py "+ corpus.devdata +" "+pfx+" > evaluable.txt";
	...
	const vector<int> actions;
        ...
        for (unsigned sii = 0; sii < test_size; ++sii) {
           ...
           //cerr << "Loaded 2 point \n";
             ComputationGraph hg;
             //cerr <<  dwords;
             vector<unsigned> pred = parser.log_prob_parser(&hg,sentence,actions,0,false,false);
             //cerr << "Loaded 4 point \n";
             double lp = as_scalar(hg.incremental_forward());
             //cerr << "Loaded 5 point \n";
             int ti = 0;
